...
export default function (): Record<string, Record<string, DefinitionRpc>> {
	...
	const numberParam: DefinitionRpcParam = {
		isOptional: true,
		name: 'number',
		type: 'BlockNumber',
	};
	const hashParam: DefinitionRpcParam = { name: 'hash', type: 'H256' };
	const reqParam: DefinitionRpcParam = {
		name: 'request',
		type: 'CallRequest',
	};
	const blockParam: DefinitionRpcParam = {
		name: 'block',
		type: 'BlockNumber',
	};
	...
	return {
		eth: {
			accounts: generateDescription(
				'Returns accounts list.',
				'Vec<H160>'
			),
			blockNumber: generateDescription(
			...
			estimateGas: generateDescription(
				'Estimate gas needed for execution of given contract.',
				'U256',
				[reqParam, numberParam]
			),
			gasPrice: generateDescription('Returns current gas_price.', 'U256'),
			getBalance: generateDescription(
				'Returns balance of the given account.',
				'U256',
				[addressParam, numberParam]
			),
			getBlockByHash: generateDescription(
				'Returns block with given hash.',
				'RichBlock',
				[hashParam, { isOptional: true, name: 'full', type: 'bool' }]
			),
			getBlockByNumber: generateDescription(
				'Returns block with given number.',
				'RichBlock',
				[blockParam, { isOptional: true, name: 'full', type: 'bool' }]
			),
			...
			getCode: generateDescription(
				'Returns the code at given address at given time (block number).',
				'Bytes',
				[addressParam, numberParam]
			),
			getLogs: generateDescription(
				'Returns logs matching given filter object.',
				'Vec<Log>',
				[{ name: 'filter', type: 'Filter' }]
			),
			getStorageAt: generateDescription(
				'Returns content of the storage at given address.',
				'H256',
				[addressParam, indexParam, numberParam]
			),
			...
			getTransactionByHash: generateDescription(
			...
			getTransactionReceipt: generateDescription(
				'Returns transaction receipt by transaction hash.',
				'Receipt',
				[hashParam]
			),
			getUncleByBlockHashAndIndex: generateDescription(
				'Returns an uncles at given block and index.',
				'RichBlock',
				[hashParam, indexParam]
			),
			getUncleByBlockNumberAndIndex: generateDescription(
				'Returns an uncles at given block and index.',
				'RichBlock',
				[numberParam, indexParam]
			),
			...
			submitHashrate: generateDescription(
				'Used for submitting mining hashrate.',
				'bool',
				[indexParam, hashParam]
			),
			submitWork: generateDescription(
			...
			syncing: generateDescription(
				'Returns an object with data about the sync status or false. (wtf?)',
				'SyncStatus'
			),
			unsubscribe: generateDescription(
				'nsubscribe from existing Eth subscription.',
				'',
				[
					{ name: '_metadata', type: 'Self::Metadata' },
